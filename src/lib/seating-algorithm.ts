...
export function allocateSeating(
  ...
  // ============================================================
  {
    ...
    // ----------------------------------------------------------------
    // PHASE 2 — GROUP C & D FILLER:
    // For every leftover code still in the pool, walk room-by-room and
    // fill Group C (9 seats) first, then Group D (6 seats) of the SAME
    // room using the SAME exam code. When that code is exhausted, pick
    // the next leftover code and continue from where we left off.
    // A code's students are kept contiguous (largest dept first).
    // No splitting rule here — codes are sliced naturally across the
    // C → D → next-room-C → next-room-D walk.
    // ----------------------------------------------------------------
    {
      const usedC: number[] = Array(rooms.length).fill(0);
      const usedD: number[] = Array(rooms.length).fill(0);

      // Flatten all leftover students in code order (largest code first)
      const cdQueue: StudentRecord[] = [];
      const orderedPool = [...pool].sort((a, b) => b.count - a.count);
      for (const code of orderedPool) {
        for (const s of code.students) cdQueue.push(s);
      }

      let qIdx = 0;
      for (let ri = 0; ri < rooms.length && qIdx < cdQueue.length; ri++) {
        for (const group of ['C', 'D'] as const) {
          const slotList = roomSlots[ri][group];
          const usedArr = group === 'C' ? usedC : usedD;
          while (usedArr[ri] < slotList.length && qIdx < cdQueue.length) {
            const pos = slotList[usedArr[ri]];
            rooms[ri].grid[pos.row][pos.col] = cdQueue[qIdx];
            rooms[ri].students.push(cdQueue[qIdx]);
            usedArr[ri]++;
            qIdx++;
          }
        }
      ...
      // If queue still has students, append new rooms and continue
      while (qIdx < cdQueue.length) {
        const i = rooms.length;
        const grid: (StudentRecord | null)[][] = Array.from(
          { length: rows },
          () => Array(totalCols).fill(null)
        ...
        for (const group of ['C', 'D'] as const) {
          const slotList = roomSlots[i][group];
          const usedArr = group === 'C' ? usedC : usedD;
          while (usedArr[i] < slotList.length && qIdx < cdQueue.length) {
            const pos = slotList[usedArr[i]];
            rooms[i].grid[pos.row][pos.col] = cdQueue[qIdx];
            rooms[i].students.push(cdQueue[qIdx]);
            usedArr[i]++;
            qIdx++;
          }
        }
