...
export function allocateSeating(
  ...
  // ============================================================
  // BIG-CODE SLACK FILL (per-group, A and B independently):
  // After A/B reservations are placed, each group may have rooms
  // with trailing empty seats (e.g. 15-seat group used only 11).
  // For each such gap, pull students from the NEXT big code that
  // was reserved to the SAME group, in reservation order. That
  // next code STILL starts in its own fresh room — we are only
  // moving its TAIL students backward into the prior slack.
  //
  // To preserve "departments stay contiguous within a code" and
  // "minor depts end up last", we pull from the END of the next
  // code's queue (its smallest-dept tail) into the slack.
  // ============================================================
  for (const targetGroup of ['A', 'B'] as const) {
    const groupRes = reservations.filter((r) => r.group === targetGroup);
    const groupSize = targetGroup === 'A' ? groupASize : groupBSize;
    const usedArr = targetGroup === 'A' ? usedA : usedB;

    for (let i = 0; i < groupRes.length - 1; i++) {
      const cur = groupRes[i];
      const next = groupRes[i + 1];

      // Collect empty slots in cur's last room(s) for this group
      const emptySlots: { roomIdx: number; slotIdx: number }[] = [];
      for (let rOffset = 0; rOffset < cur.roomCount; rOffset++) {
        const roomIdx = cur.startRoom + rOffset;
        for (let s = usedArr[roomIdx]; s < groupSize; s++) {
          emptySlots.push({ roomIdx, slotIdx: s });
        }
      }
      if (emptySlots.length === 0) continue;

      // Pull from the TAIL of next code's currently placed students
      // in its reserved rooms for this same group.
      const nextOccupied: { roomIdx: number; slotIdx: number }[] = [];
      for (let rOffset = 0; rOffset < next.roomCount; rOffset++) {
        const roomIdx = next.startRoom + rOffset;
        for (let s = 0; s < usedArr[roomIdx]; s++) {
          nextOccupied.push({ roomIdx, slotIdx: s });
        }
      }

      const moveCount = Math.min(emptySlots.length, nextOccupied.length);
      // We want next code's LAST seats (tail = smallest depts) to move.
      // Take from end of nextOccupied; place into earliest emptySlots.
      for (let k = 0; k < moveCount; k++) {
        const from = nextOccupied[nextOccupied.length - 1 - k];
        const to = emptySlots[k];

        const fromSlot = roomSlots[from.roomIdx][targetGroup][from.slotIdx];
        const toSlot = roomSlots[to.roomIdx][targetGroup][to.slotIdx];
        const student = rooms[from.roomIdx].grid[fromSlot.row][fromSlot.col];
        if (!student) continue;

        rooms[to.roomIdx].grid[toSlot.row][toSlot.col] = student;
        rooms[from.roomIdx].grid[fromSlot.row][fromSlot.col] = null;
        usedArr[to.roomIdx]++;
        usedArr[from.roomIdx]--;
      }

      // Resync students[] for affected rooms
      const affected = new Set<number>();
      for (let r = 0; r < cur.roomCount; r++) affected.add(cur.startRoom + r);
      for (let r = 0; r < next.roomCount; r++) affected.add(next.startRoom + r);
